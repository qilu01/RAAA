...
class RoleRGCNEncoder(RGCNEncoder):
  ...
  def forward(self, attn_fts, node_types, attr_order_idxs, rel_edges):
    '''Args:
      ...
      - attn_fts: (batch_size, num_src_nodes, in_feat) 128*10*2048
      - rel_edges: (num_rels, num_tgt_nodes, num_src_nodes)  128*6*10*10
      - node_types: (batch_size, num_src_nodes) 128*10
      - attr_order_idxs: (batch_size, num_src_nodes) 128*10
    '''
